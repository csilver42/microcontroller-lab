...
#include <Adafruit_NeoPixel.h>
  ...
  #include <avr/power.h>
#endif
#define LEDPIN 0 //Adafruit_RGB_Stripe
...
Adafruit_NeoPixel strip = Adafruit_NeoPixel(36, LEDPIN, NEO_GRB + NEO_KHZ800);
...
uint8_t servo01 = 0; //Door
uint8_t servo02 = 1; //Window
...
//setup RGB LED stripe
  ...
  //LCD shows "Passcode:" at first row and column
  mylcd.setCursor(1 - 1, 1 - 1);
  mylcd.print("Passcode:");
...
void loop() {
  ...
  switch (val) {
    ...
    case 'l'://if val is character 'l'，program will circulate
      //servo01 connected to digital 9 to 180°
      servos.setPWM(servo01, 0, angle2pulse(180));
      delay(500);
      break;//exit loop
    case 'm'://if val is character 'm'，program will circulate
      //servo01 connected to digital 9 to 0°
      servos.setPWM(servo01, 0, angle2pulse(0));
      delay(500);
      break;//exit loop
    case 'n'://if val is character 'n'，program will circulate
      //servo02 connected to digital 9 to 180°
      servos.setPWM(servo02, 0, angle2pulse(180));
      delay(500);
      break;//exit loop
    case 'o'://if val is character 'o'，program will circulate
      //servo02 connected to digital 9 to 0°
      servos.setPWM(servo02, 0, angle2pulse(0));
      delay(500);
      break;//exit loop
  ...
  if (button2 == 0) //if variablebutton2 is 0
  {
    ...
    if (button2 == 0)//if variablebutton2 is 0
    {
      if (passwd == ".--.-.")//if passwd is ".--.-."
      {
        ...
        mylcd.setCursor(1 - 1, 2 - 1);
        mylcd.print("open!");
      //servo01 connected to digital 9 to 100°
      servos.setPWM(servo01, 0, angle2pulse(100));
        delay(300);
        delay(5000);
        passwd = "";
        pass = "";
...
void pwm_control() {
  switch (val)
  {
    case 't'://if val is 't'，program will circulate
      ...
      servos.setPWM(servo01, 0, angle2pulse(servo1_angle));//set the angle of servo01 to servo1_angle
      delay(300);
      break;//exit loop
    case 'u'://if val is 'u'，program will circulate
      servo2 = Serial.readStringUntil('#');
      servo2_angle = String(servo2).toInt();
      servos.setPWM(servo02, 0, angle2pulse(servo2_angle));//set the angle of servo02 to servo2_angle
      delay(300);
      break;//exit loop
...
void colorWipe(uint32_t col, uint8_t wait) {
...
void theaterChase(uint32_t col, uint8_t wait) {
  for (int j=0; j<10; j++) {  //do 10 cycles of chasing
    for (int q=0; q < 3; q++) {
      for (uint16_t i=0; i < strip.numPixels(); i=i+3) {
        strip.setPixelColor(i+q, col);    //turn every third pixel on
      }
      strip.show();
